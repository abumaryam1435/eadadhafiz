...
export const ReportsTable: React.FC<ReportsTableProps> = ({ subjectFilter = 'quran' }) => {
  ...
  return (
    <div className="bg-white p-4 sm:p-6 rounded-2xl shadow-xl dark:bg-gray-800 border border-gray-100 dark:border-gray-700" ref={containerRef}>
      ...
      <div className="flex flex-wrap items-center gap-1.5 sm:gap-2 mb-6 no-print border-t border-gray-100 dark:border-gray-700 pt-5">
          <button 
            type="button"
          ...
          <button 
            type="button"
            onClick={() => exportToPdf(dynamicHeaders, sortedAndFilteredData, exportHeaderInfo.fileName, exportHeaderInfo.title, exportHeaderInfo.subtitle, hijriAdjustments, colorMap)} 
          ...
          <button 
            type="button"
            onClick={() => sharePdfDirectly(dynamicHeaders, sortedAndFilteredData, exportHeaderInfo.fileName, exportHeaderInfo.title, exportHeaderInfo.subtitle, hijriAdjustments, colorMap, undefined, "landscape")} 
          ...
          {currentUser?.role === UserRole.SUPERVISOR && (
            <button 
              ...
              className={`px-3 py-2 text-xs font-black rounded-xl shadow-xs transition-all flex items-center justify-center gap-1.5 shrink-0 whitespace-nowrap ${isQuickEditActive ? 'bg-amber-600 text-white ring-2 ring-amber-300 animate-pulse' : 'bg-amber-100 text-amber-800 border border-amber-300 dark:bg-amber-900/40 dark:text-amber-200'}`}
            >
                <span>{isQuickEditActive ? 'حفظ البيانات' : 'تعديل سريع'}</span>
            </button>
          )}
          ...
          {!topStudentsSortDesc && (
              <div ref={columnPickerRef} className="relative inline-block text-right shrink-0">
                    ...
                    >
                      <svg className="w-3.5 h-3.5 text-gray-500 dark:text-gray-300 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 17V7m0 10a2 2 0 01-2 2H5a2 2 0 01-2-2V7a2 2 0 012-2h2a2 2 0 012 2m0 10a2 2 0 002 2h2a2 2 0 002-2M9 7a2 2 0 012-2h2a2 2 0 012 2m0 10V7m0 10a2 2 0 002 2h2a2 2 0 002-2V7a2 2 0 00-2-2h-2a2 2 0 00-2 2"/></svg>
                      <span>الأعمدة</span>
                    </button>
                    {showColumnPicker && (
                        <>
                ...
                <tr key={item.id} className={`${idx % 2 === 0 ? "" : "bg-gray-50/20 dark:bg-gray-900/5"} hover:bg-green-50/30 transition-colors group`}>
                  {dynamicHeaders.map(h => {
                    ...
                    if (isEditable && h.key === 'weekNumber') {
                      ...
                      cellContent = <select value={currentHalaqaId} onChange={(e) => handleDraftUpdate(item.id, 'halaqaId', e.target.value)} className="p-1.5 text-xs sm:text-sm border-2 border-amber-400 bg-amber-50 dark:bg-gray-700 dark:border-amber-500 rounded-lg font-bold text-gray-900 dark:text-white"><option value="0">معلم متنقل</option>{[...halaqas].sort((a,b) => a.name.localeCompare(b.name, 'ar', { numeric: true })).map(h => <option key={h.id} value={h.id}>{h.name}</option>)}</select>;
                      ...
                      cellContent = (
                        <select
                          ...
                          className="p-1.5 text-xs sm:text-sm border-2 border-amber-400 bg-amber-50 dark:bg-gray-700 dark:border-amber-500 rounded-lg font-bold text-gray-900 dark:text-white focus:ring-2 focus:ring-amber-500"
                        >
                      ...
                      cellContent = (
                        <input
                          ...
                          className="w-16 p-1.5 text-xs sm:text-sm text-center border-2 border-amber-400 bg-amber-50 dark:bg-gray-700 dark:border-amber-500 rounded-lg font-bold text-gray-900 dark:text-white focus:ring-2 focus:ring-amber-500 selection:bg-amber-300 selection:text-amber-950"
                        />
                      );
                      ...
                      cellContent = (
                        <input
                          ...
                          placeholder="ملاحظات..."
                          className="w-36 sm:w-48 p-1.5 text-xs sm:text-sm border-2 border-amber-400 bg-amber-50 dark:bg-gray-700 dark:border-amber-500 rounded-lg font-medium text-gray-900 dark:text-white focus:ring-2 focus:ring-amber-500 selection:bg-amber-300 selection:text-amber-950"
                        />
                      );
                  ...
                  {!topStudentsSortDesc && (
                    <td className="px-3.5 py-3 whitespace-nowrap text-center no-print">
                        <div className="flex justify-center gap-2">
                            <button onClick={() => handleEdit(item)} disabled={isQuickEditActive} className={`p-2 rounded-lg transition-colors ${isQuickEditActive ? 'text-gray-300 opacity-50' : 'text-blue-600 hover:bg-blue-50 border border-blue-100 dark:border-gray-600 dark:hover:bg-blue-900/30'}`} title="تعديل"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg></button>
                            <button onClick={() => handleDelete(item.id)} disabled={item.attendance === NOT_RECORDED || isQuickEditActive} className={`p-2 rounded-lg transition-colors ${item.attendance === NOT_RECORDED || isQuickEditActive ? 'text-gray-300 opacity-50' : 'text-red-600 hover:bg-red-50 border border-red-100 dark:border-gray-600 dark:hover:bg-red-900/30'}`} title="حذف"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg></button>
                        </div>
                    </td>
                  )}
                </tr>
