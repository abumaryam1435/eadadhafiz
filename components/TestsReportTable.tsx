  ...
  const TestsSummaryMatrix: React.FC<{ 
    ...
    return (
        <>
            ...
            <div className="flex flex-wrap items-center gap-1.5 sm:gap-2 mb-4 no-print border-t pt-4 border-gray-100 dark:border-gray-700">
                <button 
                  type="button"
                  onClick={() => setIsSummaryExcelModalOpen(true)} 
                ...
                <button 
                  type="button"
                  onClick={() => setIsSummaryWordModalOpen(true)} 
                ...
                <button 
                  type="button"
                  onClick={() => exportToPdf(filteredHeaders, filteredData.filter(r => r.rank !== -1), 'ملخص_الاختبارات', 'تقرير ملخص الاختبارات والمستويات', undefined, hijriAdjustments, colorMap, rankColors)} 
                ...
                <button 
                  type="button"
                  onClick={() => sharePdfDirectly(filteredHeaders, filteredData.filter(r => r.rank !== -1), 'ملخص_الاختبارات', 'تقرير ملخص الاختبارات والمستويات', undefined, hijriAdjustments, colorMap, rankColors, "landscape")} 
                ...
                <button
                  ...
                  className={`px-3 py-2 text-xs font-bold rounded-xl shadow-xs flex items-center justify-center gap-1.5 transition-all shrink-0 whitespace-nowrap ${isQuickEdit ? 'bg-indigo-600 text-white dark:bg-indigo-500' : 'text-gray-700 bg-indigo-50 dark:bg-indigo-900/30 dark:text-indigo-200'}`}
                >
                    <span>{isQuickEdit ? 'تعديل سريع: مفعل' : 'تعديل سريع'}</span>
                </button>
                ...
                <div className="relative inline-block text-right shrink-0">
                    <button 
                      ...
                      className="px-2.5 sm:px-3 py-2 text-xs font-bold text-yellow-800 bg-yellow-50 dark:bg-yellow-900/30 dark:text-yellow-200 border border-yellow-200 dark:border-yellow-700/50 rounded-xl shadow-xs flex items-center justify-center gap-1.5 hover:bg-yellow-100 transition-colors shrink-0 whitespace-nowrap cursor-pointer"
                    >
                      <span>🎨</span>
                      <span>ألوان المراتب</span>
                    </button>
                </div>
                ...
                <button 
                  type="button"
                  onClick={handleSync} 
                  className={`px-2.5 sm:px-3 py-2 text-xs font-bold rounded-xl shadow-xs flex items-center justify-center gap-1.5 transition-all duration-300 shrink-0 whitespace-nowrap cursor-pointer ${isCloudSynced ? 'bg-orange-100 text-orange-700 dark:bg-orange-900/30 dark:text-orange-400' : 'text-white bg-indigo-600 hover:bg-indigo-700'}`}
                >
                    {isCloudSynced ? (
                        <>
    ...
    <div className="bg-white p-4 sm:p-6 rounded-2xl shadow-xl dark:bg-gray-800 border border-gray-100 dark:border-gray-700" ref={containerRef}>
      ...
      <div className={activeTab === 'detailed' ? 'block' : 'hidden'}>
          ...
          <div className="flex flex-wrap items-center gap-1.5 sm:gap-2 mb-6 no-print border-t border-gray-100 dark:border-gray-700 pt-5">
              <button 
                type="button"
                onClick={() => setIsDetailedExcelModalOpen(true)} 
                className="px-2.5 sm:px-3 py-2 text-xs font-bold text-white bg-green-600 hover:bg-green-700 rounded-xl shadow-xs active:scale-95 transition-all flex items-center justify-center gap-1.5 cursor-pointer shrink-0 whitespace-nowrap"
                title="تصدير ملف Excel"
              >
                <svg className="w-3.5 h-3.5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
              ...
              <button 
                type="button"
                onClick={() => setIsDetailedWordModalOpen(true)} 
                className="px-2.5 sm:px-3 py-2 text-xs font-bold text-white bg-blue-600 hover:bg-blue-700 rounded-xl shadow-xs active:scale-95 transition-all flex items-center justify-center gap-1.5 cursor-pointer shrink-0 whitespace-nowrap"
                title="تصدير ملف Word"
              >
                <svg className="w-3.5 h-3.5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
              ...
              <button 
                type="button"
                onClick={() => exportToPdf(dynamicHeaders, sortedAndFilteredData, exportHeaderInfo.fileName, exportHeaderInfo.title, exportHeaderInfo.subtitle, hijriAdjustments, colorMap, rankColors)} 
                className="px-2.5 sm:px-3 py-2 text-xs font-bold text-white bg-red-600 hover:bg-red-700 rounded-xl shadow-xs active:scale-95 flex items-center justify-center gap-1.5 transition-all cursor-pointer shrink-0 whitespace-nowrap"
                title="طباعة وتصدير ملف PDF"
              >
                <svg className="w-3.5 h-3.5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h10z"></path></svg>
              ...
              <button 
                type="button"
                onClick={() => sharePdfDirectly(dynamicHeaders, sortedAndFilteredData, exportHeaderInfo.fileName, exportHeaderInfo.title, exportHeaderInfo.subtitle, hijriAdjustments, colorMap, rankColors, "landscape")} 
                className="px-2.5 sm:px-3 py-2 text-xs font-bold text-white bg-emerald-600 hover:bg-emerald-700 rounded-xl shadow-xs active:scale-95 flex items-center justify-center gap-1.5 transition-all cursor-pointer shrink-0 whitespace-nowrap"
                title="مشاركة تقرير PDF"
              >
                <svg className="w-3.5 h-3.5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M8.684 13.342C8.886 12.938 9 12.482 9 12c0-.482-.114-.938-.316-1.342m0 2.684a3 3 0 110-2.684m0 2.684l6.632 3.316m-6.632-6l6.632-3.316m0 0a3 3 0 105.367-2.684 3 3 0 00-5.367 2.684zm0 9.316a3 3 0 105.368 2.684 3 3 0 00-5.368-2.684z"/></svg>
