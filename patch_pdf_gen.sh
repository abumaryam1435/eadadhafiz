#!/bin/bash
cat << 'INNER_EOF' > new_pdf_draw.txt
...
sed -i -e '/ctx.clearRect(0, 0, canvas.width, canvas.height);/,/\/\/ Re-calculate font size relative to height for scaling/c\' -e "$(sed 's/$/\\/g' new_pdf_draw.txt | sed '$s/\\//')" components/CardsManager.tsx
